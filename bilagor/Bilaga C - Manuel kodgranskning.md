...
| -     | Bristande HTTP Security Headers              | A02 Security Misconfiguration | Spring Security lägger till vissa headers automatiskt men ingen explicit konfiguration finns för t.ex. `X-Content-Type-Options`, `X-Frame-Options` eller `X-XSS-Protection`.                            | Konfigurera security headers explicit i filterkedjan. |
